...
s = hoomd.init.read_gsd("../Sim_dump_ribbon_wall/init_strip.gsd")
...
walls.add_plane(origin=(100.5,0,0),normal=(-1,0,0),inside=True)
...
lj.force_coeff.set('A', sigma=1.0,epsilon=0.0, r_cut=75.0)
lj.force_coeff.set('C', sigma=1.0,epsilon=0.0, r_cut=75.0)
lj.force_coeff.set('D', sigma=1.0,epsilon=5.0, r_cut=75.0)


hoomd.analyze.log(filename="../Sim_dump_ribbon_wall/observable.log", quantities=["temperature", "potential_energy","bond_harmonic_energy","kinetic_energy","dihedral_harmonic_energy"], period=5000, header_prefix="#", overwrite=True)
...
hoomd.dump.gsd(filename="../Sim_dump_ribbon_wall/trajectory.gsd", group=group.all(), period=5000, overwrite=True,static=[])
